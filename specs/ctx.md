...
- [x] Verify + hand over: `node --test` green (no gate changes); user runs switch; fresh session — footer is ONE line, segment numbers match the default footer's, `/footer` toggles back, narrow-terminal truncation sane.
## Findings
...
## Next
Handover done — user runs switch, then fresh-session checks: footer is ONE line (`~/… · ↑in ↓out · pct/win · model · mode`), numbers match the default footer's, `/footer` toggles, narrow terminals truncate sanely. Note: ⚙ implement now always shows in the footer (was cleared before).
